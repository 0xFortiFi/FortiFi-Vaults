...
contract FortiFiMASSVault is IMASS, ERC1155Supply, IERC1155Receiver, Ownable, ReentrancyGuard {
    ...
    function deposit(
        uint256 _amount
    ) external override nonReentrant whileNotPaused returns (uint256 _tokenId, TokenInfo memory _info) {
    ...
    function add(
        uint256 _amount,
        uint256 _tokenId
    ) external override nonReentrant whileNotPaused returns (TokenInfo memory _info) {
        if (_amount < minDeposit) revert InvalidDeposit();
        IERC20(depositToken).safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function setStrategies(Strategy[] memory _strategies) public onlyOwner {
        ...
        for (uint256 i = 0; i < _length; i++) {
            ...
            if (_strategies[i].decimals <= USDC_DECIMALS && _strategies[i].depositToken != depositToken)
    ...
    function _swapFromDepositToken(uint256 _amount, Strategy memory _strat) internal returns (uint256) {
        ...
        uint256 _swapAmount = ((_amount * (10 ** _strat.decimals)) / _latestPrice) *
            10 ** (_strat.decimals - USDC_DECIMALS);
    ...
    function _swapToDepositToken(uint256 _amount, Strategy memory _strat) internal {
        ...
        uint256 _latestPrice = _oracle.getPrice();
        uint256 _swapAmount = (_amount * (_latestPrice / 10 ** _strat.decimals)) /
            10 ** (_strat.decimals - USDC_DECIMALS);
        ...
        for (uint256 i = 0; i < _length; i++) {
            ...
            if (_isSAMS) {
                ...
                if (_isAdd) {
                    ...
                    _info.positions.push(
                        Position({strategy: _strategy, receipt: _strat.balanceOf(address(this)) - _receiptBalance})
                    );
                }
            }
        }
    ...
    function _withdraw(uint256 _tokenId) internal returns (uint256 _proceeds, uint256 _profit) {
        ...
        for (uint256 i = 0; i < _length; i++) {
            ...
            if (_info.positions[i].strategy.depositToken != depositToken) {
                uint256 _strategyDepositTokenProceeds = IERC20(_info.positions[i].strategy.depositToken).balanceOf(
                    address(this)
                );
