...
import "./FortiFiGLPFortressArb.sol";
...
/// @title GLP FortiFi Strategy contract for Arbitrum
...
contract FortiFiGLPStrategyArb is FortiFiStrategy {
    ...
    address public rewardRouter = 0xB95DB5B167D75e6d04227CfFFA61069348d271F5;
    ...
    function depositToFortress(uint256 _amount, address _user, uint256 _tokenId) external override {
        ...
        if (vaultToTokenToFortress[msg.sender][_tokenId] == address(0)) {
            FortiFiGLPFortressArb _fort = new FortiFiGLPFortressArb(_strat, stakedGLP, address(_dToken), address(_wNative), rewardRouter);
